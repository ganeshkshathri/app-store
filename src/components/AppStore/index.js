...
class AppStore extends Component {
  state = {
    isApp: tabsList[0].tabId,
    isList: appsList,
  }

  selectCategory = value => {
    this.setState({isApp: value})
  }

  onSearchValue = event => {
    const appsFilter = appsList.filter(eachFilter =>
      eachFilter.appName
        .toLowerCase()
        .includes(event.target.value.toLowerCase()),
    )
    console.log(appsFilter)
    this.setState({isList: appsFilter})
  }

  render() {
    const {isApp, isList} = this.state
    const searchCategory = isList.filter(eachCategory =>
      eachCategory.category.includes(isApp),
    )
      ...
      <div className="main-container">
        <div className="sub-container">
          ...
          <div className="search-container">
            <input type="search" onChange={this.onSearchValue} />
            <img
          ...
          <ul className="unordered-tab">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={eachTab.tabId === isApp}
              />
            ))}
          </ul>
          <ul className="unordered-app">
            {searchCategory.map(eachApp => (
